...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			//If account doesn't exisist create one
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		//if (accountlist.containsKey(accountid)) { //should throw error when account not exisist, now it works :D
		if (!accountlist.containsKey(accountid)) {
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			//account.deposit(money); in this func we want to withdraw not deposit
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		//transfer(fromaccount, this, fromaccount, amount); transfer should be from sth to sth not from twice
...
// in addTimedPaymente and removeTimedPayment is missing the same part : there is no exception when account does not exists
